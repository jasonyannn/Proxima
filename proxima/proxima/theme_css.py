...
"""The stylesheet, and the font links it needs.

Lifted out of theme.py so that file stays about the design system — the tokens,
and the helpers that draw with them — rather than being one long string with
some Python around it. Nothing imports this but ``theme.inject``.

``__VARS__`` is substituted with the custom properties built from the tokens;
see ``theme._vars``.
"""
